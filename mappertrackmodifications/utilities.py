...
def run(command, params=None, ignore_errors=False, print_output=True, print_time=False, working_dir=None, containers=None): # , toolkit=None
    ...
    fsl_container = join(containers_dir,'fsl-v6.0.sif') if containers is not None else None 
    fs_container = join(containers_dir,'freesurfer-v7.4.1.sif') if containers is not None else None
    mrtrix3_container = join(containers_dir,'mrtrix3-v3.0.3.sif') if containers is not None else None
    fsl_commands = ['fsl', 'flirt', 'convert_xfm', 'topup', 'eddy', 'bet', 'dtifit', 'bedpostx', 'make_dyadic_vectors', 'probtrackx2', 'find_the_biggest', 'find', 'sh', 'cat','bedpostx_gpu']
    ...
    if (container is not None) or (containers is not None):
        ...
        command = (f'singularity exec {"--nv" if use_gpu else ""} ' +
            ...
            # f'-B /opt/sge ' +
            # WE MODIFIED THE LINE BELOW TO BE OUR PATH 
            f'-B /ceph/chpc/shared/shinjini_kundu_group/working/yash/mappertraccontainers/license.txt:/usr/local/freesurfer/7.4.1/license.txt ' + # we should be able to replace this hard-coded host path for license to a param
            f'-B {work_dir}:/mappertrac {container_run} ' +
            # bellow was modified so the SUBJECTS_DIR env var is correctly modified the work_dir under sub/ses
            f'sh -c "pwd ; export SUBJECTS_DIR=/mappertrac ; {command}"')
        print(command)
        if container_cwd:
