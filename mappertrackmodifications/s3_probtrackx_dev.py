...
def conclude(params, inputs=[]):
    ...
    # New way of computing FAtractsumsRaw
    write(stdout, "Using the new way to compute FAtractsumsRaw:")
    pbtx_merge = join(edi_maps, "FAtractsumsRaw_merge.nii.gz")
    pbtx_mean = join(edi_maps, "FAtractsumsRaw_mean.nii.gz")
    pbtx_all = join(pbtk_dir, "*.nii.gz")
    pbtx_listdir = os.listdir(pbtk_dir)
    nPbtx = len(pbtx_listdir) - 2
    run("fslmerge -t {0} {1}".format(pbtx_merge, pbtx_all), params)
    run("fslmaths {0} -Tmean {1}".format(pbtx_merge, pbtx_mean), params)
    run("fslmaths {0} -mul {1} {2}".format(pbtx_mean, nPbtx, tract_total), params)
    ...
    edge_mean = join(edi_maps, "FAtractsumsTwoway_mean.nii.gz")
    consensus_all = join(consensus_dir, "*.nii.gz")
    nEdges = len(consensus_edges)
    run("fslmerge -t {0} {1}".format(edge_merge, consensus_all), params)
    run("fslmaths {0} -Tmean {1}".format(edge_merge, edge_mean), params)
    run("fslmaths {0} -mul {1} {2}".format(edge_mean, nEdges, edge_total), params)
