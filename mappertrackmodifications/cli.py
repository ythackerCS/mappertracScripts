...
def main():
    ...
    for input_dir in session_dirs:
        ...
        #removed 'derivatives' in the join that was adding redundant derivative folder
        subject_dir = join(output_dir,subject, session)
        param = base_params.copy()
        param.update({
            #modified from just input_dir to join with work,dir 
            'input_dir': join(input_dir, 'mappertrac' ,'work_dir'),
            #NOTE THIS WAS MODIFIED TO ADD EXTRA MAPPERTRAC dir 
            'work_dir': join(subject_dir,  'mappertrac', 'work_dir'),
            'ID': f'{subject}_{session}',
            'stdout': join(subject_dir, 'worker.stdout'),
        })
        all_params.append(param)

    #WE SHOULDNT GO INTO THIS LOOP SO WE AREN'T MODIFYING
    for input_dir in subject_dirs:
        subject = basename(input_dir)
        #similar structure to above forloop followed
    ...
    if args.probtrackx:
        cores_per_worker = 1
        #Below was modified from 0.1 to 5 (there is some weirdness where the core isnt getting enough mem) TOFIX? 
        mem_per_worker = 5
        ...
        #NOTE THIS WAS MODIFIED from int(os.cpu_count())
        cores_per_worker = 1
        #USED TO BE None 
        mem_per_worker = 5
    ...
    #THIS IS FOR DEALING WITH STEP TWO THAT NEEDS GPU (MAY NEED WORK IN THE FUTURE) 
    if args.bedpostx:
        scheduler_opt=f"#SBATCH -A {args.bank}\n#SBATCH --gpus-per-node=1"
    else:
        scheduler_opt=f"#SBATCH -A {args.bank}\n"

    #NOTE THIS WAS ALSO MODIFIED TO REMOVE EXCLUSIVE FLAG 
    if args.slurm:
        executor = parsl.executors.HighThroughputExecutor(
            ...
            provider=parsl.providers.SlurmProvider(
                ...
                cores_per_node=int(args.nnodes),
                mem_per_node=40, 
                nodes_per_block=1,
                init_blocks=1,
                max_blocks=1,
                scheduler_options=scheduler_opt,
                exclusive=False,
