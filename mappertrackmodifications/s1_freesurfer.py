...
'''
    ...
    #--- Beginning of modification to match Delta version. ---#
    #note this was updated to sdir and 'dwi' was removed 
    #input_dwis = glob(join(sdir, f'{ID}_*dwi.nii.gz'))
    ...
    #input_rev = join(sdir, f'{ID_full}_dwi_rev.nii.gz') # this may need reversal of the dir-{dir} tag in the ID_full string
    #input_bval = join(sdir, f'{ID_full}_dwi.bval')
    #input_bvec = join(sdir, f'{ID_full}_dwi.bvec')    
    #if isfile(join(sdir, f'{ID}_run-01_T1w.nii.gz')):
    #    input_T1 = join(sdir,  f'{ID}_run-01_T1w.nii.gz')
    #else:
    ...
    #note input rev was not previously defined in modification added
    input_rev = join(sdir, f'{ID}_dir-rev_dwi.nii.gz')

    #--- The end of modification to match Delta version. ---#
    ...
    if exists(data_eddy):
        ...
        #NOTE: This was changed to mrisynthstrip but BET_MASK var names are kept the same (TBD) 
        #NOTE: WE ALSO CHANGED THIS TO B0 Volume so that we dont do it for all 33 vols 
        run(f'mri_synthstrip -i {b0_avg} -o {bet} -m {bet_mask}', params)
    ...
    if exists(EDI):
        ...
        # modified to make sure we are in work_dir "mappertrac" because this isnt happening TOFIX 
