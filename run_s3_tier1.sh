...
current_job=${bids_dir}/misc/jobs/s3_${sub}_${ses}.sh

if [[ -e ${current_job} ]]; then
          echo "MaPPeRTrac step 3 job for ${sub} ${ses} already exists. Remove it and rewrite."
          rm -f ${current_job}
fi

# THIS IS FOR THE PRIMAR JOB 
echo "MaPPeRTrac step 3 job for ${sub} ${ses} is being written..."

echo "#!/bin/bash" | tee -a ${current_job}

echo "#SBATCH --job-name=mapper-s3" | tee -a ${current_job}
...
echo "#SBATCH -e ${bids_dir}/misc/output/${sub}_${ses}_s3_slurm-%j.err" | tee -a ${current_job}
echo "#SBATCH -o ${bids_dir}/misc/output/${sub}_${ses}_s3_slurm-%j.out" | tee -a ${current_job}
...
#HOW TO RUN
#./run_s3_free.sh sub-14706x320YTTRY ses-SCAP1
